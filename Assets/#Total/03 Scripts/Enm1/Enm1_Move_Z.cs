...
public class Enm1_Move_Z : MonoBehaviour
{
    ...
    void Update()
    {
        if (movable)
        {
            ...
            transform.position -= transform.forward * Time.deltaTime * (1 / time); //앞으로 이동
            rotation.transform.Rotate(-90 * Time.deltaTime * (1 / time), 0, 0); //앞으로 구르기
            ...
            else if (currTime > time) //구르면서 이동한 운동 초과량 마이너스 보정
            {
                ...
                currTime = 0;
                moveCnt++;
                rotation.transform.Rotate(90 * f * (1 / time), 0, 0); //회전 초과량 보정
                transform.position += transform.forward * f * (1 / time); //이동 초과량 보정
            }
        }
        else 
        {
